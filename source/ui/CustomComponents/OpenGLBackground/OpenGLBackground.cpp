...
void OpenGLBackground::resized () {
    ...
    int xyPadX = getWidth() * 120 / 700;
    int xyPadY = getHeight() * 94 / 648;
    int xyPadWidth = getWidth() * 500 / 700;
    int xyPadHeight = getHeight() * 500 / 648;
    xyPad.setBounds(xyPadX, xyPadY, xyPadWidth, xyPadHeight);

    backgroundColor_juce = juce::Colour::fromString(ColorPallete::BG);

    int openGLStatusLabelReduced = getWidth() * 4 / 700;
    ...
    int attackWidth = getWidth() * 70 / 700;
    int attackHeight = getHeight() * 19 / 648;
    labels.attack.setBounds(attackX, attackY, attackWidth, attackHeight);

    int sharpX = getWidth() * 347 / 700;
    int sharpY = getHeight() * 60 / 648;
    int sharpWidth = getWidth() * 70 / 700;
    int sharpHeight = getHeight() * 19 / 648;
    labels.sharp.setBounds(sharpX, sharpY, sharpWidth, sharpHeight);

    int sustainX = getWidth() * 636 / 700;
    int sustainY = getHeight() * 335 / 648;
    int sustainWidth = getWidth() * 70 / 700;
    int sustainHeight = getHeight() * 19 / 648;
    labels.sustain.setBounds(sustainX, sustainY, sustainWidth, sustainHeight);

    int smoothX = getWidth() * 340 / 700;
    int smoothY = getHeight() * 605 / 648;
    int smoothWidth = getWidth() * 70 / 700;
    int smoothHeight = getHeight() * 19 / 648;
    labels.smooth.setBounds(smoothX, smoothY, smoothWidth, smoothHeight);
...
{
    auto font = CustomFontLookAndFeel::getCustomFont().withHeight(getHeight() * 19 / 648);
